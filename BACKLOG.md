...
The `hwp-training/1.0.0-sqliq` exploit already drives the `/sqliq` lab endpoint
standalone (via its `--query` option); the payload above is the reusable path for
the ~77 real `SQLIq` exploits.
